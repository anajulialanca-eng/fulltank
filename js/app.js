...
$('maintenanceForm').onsubmit=async e=>{e.preventDefault();const vehicleId=$('maintenanceVehicle').value,editId=e.target.dataset.editId;if(!vehicleId)return alert('Cadastre um veículo primeiro.');const data={vehicleId,type:$('maintenanceType').value,desc:$('maintenanceDesc').value.trim(),doneDate:$('maintenanceDoneDate').value||today(),place:$('maintenancePlace').value.trim(),lastKm:parseBR($('maintenanceLastKm').value),nextKm:parseBR($('maintenanceNextKm').value),nextDate:$('maintenanceDate').value,cost:parseBR($('maintenanceCost').value),notes:$('maintenanceNotes').value.trim()};try{if(editId){await updateDoc(doc(db,'garages',garage.id,'maintenance',editId),{...data,updatedAt:serverTimestamp()});delete e.target.dataset.editId;msg('maintenanceMsg','Cuidado atualizado com sucesso.','ok')}else{await addDoc(collection(db,'garages',garage.id,'maintenance'),{...data,createdBy:user.uid,createdByName:profile.name,createdAt:serverTimestamp()});msg('maintenanceMsg','Manutenção salva e sincronizada.','ok')}e.target.reset()}catch(err){console.error(err);msg('maintenanceMsg','Não foi possível salvar.','err')}};
function editMaintenance(r){$('maintenanceVehicle').value=r.vehicleId;$('maintenanceType').value=r.type||'Outro';$('maintenanceDesc').value=r.desc||'';$('maintenanceDoneDate').value=r.doneDate||today();$('maintenancePlace').value=r.place||'';$('maintenanceLastKm').value=numInput(r.lastKm);$('maintenanceNextKm').value=numInput(r.nextKm);$('maintenanceDate').value=r.nextDate||'';$('maintenanceCost').value=numInput(r.cost);$('maintenanceNotes').value=r.notes||'';$('maintenanceForm').dataset.editId=r.id;msg('maintenanceMsg','Editando cuidado. Altere os dados e salve.','ok');go('maintenance');$('maintenanceDesc').focus()}
...
function maintenanceHTML(r,home=false){const v=state.vehicles.find(x=>x.id===r.vehicleId);return `<div class="item ${maintenanceClass(r)}"><div class="itemtop"><div><div class="date">${v?icon(v.type)+' '+esc(v.name):''} • por ${esc(r.createdByName||'usuário')}</div><strong>${esc(r.type)}</strong><div>${esc(r.desc)}</div></div>${!home&&canDelete(r)?'<div class="actions"><button class="link edit-maintenance">Editar</button><button class="link del">Excluir</button></div>':''}</div><div class="details"><span>Realizado em: ${r.doneDate?new Date(r.doneDate+'T12:00:00').toLocaleDateString('pt-BR'):'—'}</span><span>Local: ${esc(r.place||'—')}</span><span>Próximo KM: ${r.nextKm?num(r.nextKm,0):'—'}</span><span>Próxima data: ${r.nextDate?new Date(r.nextDate+'T12:00:00').toLocaleDateString('pt-BR'):'—'}</span><span>Valor: ${money(r.cost)}</span><span>${esc(r.notes||'')}</span></div></div>`}
function renderMaintenance(){const list=[...state.maintenance].sort((a,b)=>-sortCreated(a,b));$('maintenanceList').innerHTML=list.length?'':'<div class="empty">Nenhuma manutenção cadastrada.</div>';list.forEach(r=>{const w=document.createElement('div');w.innerHTML=maintenanceHTML(r);const el=w.firstElementChild;el.querySelector('.receipt-open')?.addEventListener('click',()=>el.querySelector('.history-receipt').classList.toggle('hidden'));el.querySelector('.edit-maintenance')?.addEventListener('click',()=>editMaintenance(r));el.querySelector('.del')?.addEventListener('click',async()=>{if(confirm('Excluir esta manutenção?'))await deleteDoc(doc(db,'garages',garage.id,'maintenance',r.id))});$('maintenanceList').appendChild(el)})}
...
function renderCareSummary(){
  ...
  $('lastFuelCard').onclick=fuel?()=>editFueling(fuel):null;
  $('lastWashCard').onclick=wash?()=>editMaintenance(wash):null;
  $('nextCareCard').onclick=care?()=>editMaintenance(care):null;
}
