...
function renderAll(){if(!garage)return;renderVehicleSwitch();fillVehicleSelects();renderDashboard();renderHistory();renderMaintenance();renderCareSummary();const v=state.vehicles.find(x=>x.id===state.activeVehicleId);$('prevKm').value=v?numInput(currentKm(v.id)):'';calcFuel()}
